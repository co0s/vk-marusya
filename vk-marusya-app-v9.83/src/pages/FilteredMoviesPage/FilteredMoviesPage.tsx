...
const FilteredMoviesPage: React.FC = () => {
    ...
    useEffect(() => {
        if (genreName) {
            ...
            logger.debug(`[FilteredMoviesPage] Жанр: ${genreName}`);
            logger.debug(`[FilteredMoviesPage] В кэше: ${isCached ? 'Да' : 'Нет'}`);
            logger.debug(`[FilteredMoviesPage] Текущий жанр: ${currentGenre}`);
            logger.debug(`[FilteredMoviesPage] Тот же жанр: ${isSameGenre ? 'Да' : 'Нет'}`);
            ...
            {Array.isArray(moviesToDisplay) && moviesToDisplay.length > 0 && (
                <>
                    <div className={styles.movies__grid}>
                        {moviesToDisplay.map((movie: IMovie, index: number) => (
                            <SimpleMovieCard 
                                key={`filtered-${movie.id}-${index}`} 
                                id={movie.id} 
