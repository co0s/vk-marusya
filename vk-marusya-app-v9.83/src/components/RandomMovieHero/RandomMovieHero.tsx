...
const RandomMovieHero: React.FC<RandomMovieHeroProps> = ({
  ...
  const [currentImageUrl, setCurrentImageUrl] = useState(movie.posterUrl);

  // Отслеживаем смену фильма и сбрасываем состояние загрузки
  React.useEffect(() => {
    setImageLoaded(false);
    setCurrentImageUrl(movie.posterUrl);
    ...
    >
      ...
      <div className={styles.content__wrapper}>
        ...
        <div className={styles.poster__block}>
          {!imageLoaded && (
            <div className={styles.poster__loader}>
              <div className={styles.loader__spinner}></div>
            </div>
          )}
          <img
            src={
              currentImageUrl || "https://placehold.co/300x450/1A1D2E/76a9fa"
            }
            alt={movie.title}
